...
export default function SellerBookingsPage() {
  ...
  async function load(opts?: { silent?: boolean }) {
    ...
    try {
      const bookingsData = await apiFetch<BookingResponse[]>('/api/bookings')
      setBookings([...bookingsData].sort((a, b) => toTime(b.createdAt) - toTime(a.createdAt) || b.id - a.id))
      setError(null)
    } catch (e: unknown) {
      if (!opts?.silent) {
                ...
                <tbody>
                  {bookings.map((b) => (
                    <tr key={b.id}>
                      ...
                      <td style={{ color: 'var(--color-primary)', fontWeight: 600 }}>{formatMoney(b.commissionAmount)}</td>
                      <td>{b.status}</td>
