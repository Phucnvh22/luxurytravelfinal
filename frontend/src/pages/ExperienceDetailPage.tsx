...
export default function ExperienceDetailPage() {
  ...
  const experienceId = useMemo(() => Number(id), [id])
  const [experience, setExperience] = useState<Experience | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [submitting, setSubmitting] = useState(false)
  const [submitError, setSubmitError] = useState<string | null>(null)
  const [success, setSuccess] = useState<ExperienceRequestResponse | null>(null)
  const [form, setForm] = useState<Omit<ExperienceRequestCreateRequest, 'experienceId'>>({
    customerName: '',
    email: '',
    phone: '',
  ...
  async function submit() {
    ...
    try {
      ...
      const payload: ExperienceRequestCreateRequest = {
        experienceId,
      ...
      const result = await apiFetch<ExperienceRequestResponse>('/api/experience-requests', {
        method: 'POST',
          ...
          <div className="detail-grid" style={{ marginTop: 14 }}>
            <div>
              <div className="detail-carousel">
                {experience.videoUrls && experience.videoUrls.length > 0 ? (
                  experience.videoUrls.map((url, idx) => {
                    ...
                    return videoId ? (
                      <div className="detail-carousel-item" key={idx}>
                        <iframe
                          ...
                          title={`${experience.name} - Video ${idx + 1}`}
                          frameBorder="0"
                      ...
                      <div className="detail-carousel-item" key={idx}>
                        <div className="detail-hero" style={{ backgroundImage: `url(${experience.imageUrl})` }} />
                      </div>
                    )
                  })
                ) : (
              ...
              <div className="card detail-card" style={{ marginTop: 14 }}>
                <h2 className="detail-title">{experience.name}</h2>
                <div className="detail-meta">
                  <div className="pill">{formatMoney(String(experience.priceFrom))}+</div>
                </div>
            ...
            <div className="card detail-card">
              <div className="panel-title">{t('form_request_experience', 'Request this experience')}</div>
              ...
              {!isAuthenticated ? (
                <div className="card error" style={{ marginTop: 12 }}>
                  ...
                  <div className="row" style={{ marginTop: 10 }}>
                    <Link className="btn primary" to={`/login?redirect=${encodeURIComponent(`/experiences/${experienceId}`)}`}>
                      Log in now
                    </Link>
                  </div>
                </div>
              ...
              <label className="field">
                ...
                <textarea
                  ...
                  placeholder="Preferences, timing..."
                />
              </label>
